    ...
    function (BaseController, MessageToast, JSONModel, History) {
        ...
        return BaseController.extend("sap.sync.practice1.controller.View4", {
            ...
            onInit: function () {
                ...
                // var oInput = this.byId("id1"); //없는 대상을 잘못 불러옴 - undefined
                // oInput.getValue(); // 여기서 에러가 발생
            ...
            onLiveChange: function(oEVent) {
                ...
                // 값이 수정될 때마다 발동
                if( oEVent.getParameter("value") === '1'){ //1을 넣으면 값이 초기화되서 입력안됨
                oEVent.getSource().setValue();  //이걸 넣으면 값이 항상 초기화되서 입력이안됨
                //getSource()는 자기자신을 가르킨다?
                }
            }
