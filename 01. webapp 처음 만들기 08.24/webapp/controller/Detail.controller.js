sap.ui.define([
    ...
    return Controller.extend("sap.ui.demo.walkthrough.controller.Detail", {
        onInit: function () {   //onInit은 화면이 구현될때 최초 1회만 구동
        ...
        // patternMatched 이벤트에 등록한 함수, 이벤트 객체가 첫번째 파라미터로 들어옴
        // 이 함수는 이 경로에 들어올 때마다 동작함 
        //(= 직접 onInit 같이 특정조건(화면(detailView)에 들어올때마다)에서 자동으로 수행되게 만듬) 
        _onObjectMatched: function (oEvent) {
            var oArg = oEvent.getParameter("arguments");
            var sPath = window.decodeURIComponent(oArg.invoicePath);
            //console.log(oArg, sPath);
        ...
        onNavBack: function () {
            // 라우팅 기록을 가지고 있는 history 객체
            var oHistory = History.getInstance();
            // history 객체에서 이전 hash(url) 화면기록을 찾아옴 
            var sPreviousHash = oHistory.getPreviousHash();

            if (sPreviousHash !== undefined) {  // 이전 해쉬값을 가지고 있으면
                // 브라우저 화면의 window 객체가 가지고있는 화면기록의 전단계로 이동
                // 가능한 이유는 전단계의 화면을 구현한 적이 있으니까 UI5의 전 view를
                // 불러와도 문제가 없음 
                window.history.go(-1);
            } else {    // 이전 해쉬값이 없으면 
                        // pattern : "" 최초화면이 아니면
                        // 이회면에서 새로고침을 하면 이전기록이 없어짐
