sap.ui.define([
	...
	"../model/formatter", // ../ 내 폴더위치 보다 상위폴더
	"sap/ui/model/Filter",			// 필터 - 필터객체를 만들어서 데이터바인딩에 적용
	...
	return Controller.extend("sap.ui.demo.walkthrough.controller.InvoiceList", {
		...
		onFilterInvoices : function (oEvent) {
			// 이벤트 펑션의 첫번째 파라미터로 이벤트객체가 들어옴
			// 이걸 이용해서 이벤트객체를 기능 구현에 사용

			// this.getView().byId("서치필드아이디").getValue() 3step
			// 얘는 이벤트 동작 함수이기 때문에 이렇게 찾아올 필요가 없고
			...
			// 필터는 배열인 객체로 데이터바인딩에 옵션값으로 추가시킴
			...
			if (sQuery) { // [필터적용 3단계] - 필터객체를 만드는 조건은 검색어가 있을때만
				aFilter.push(
					...
					// (path(필터를 적용할 데이터바인딩 기준 경로) , operator(검색연산자) , 검색조건 )
				);
			...
			// [필터적용 6단계] - list 바인딩정보에 필터 속성도 추가
		},

		onPress: function (oEvent) {
			var oItem = oEvent.getSource();
			var oRouter = this.getOwnerComponent().getRouter();
			// controller -> compenent -> router 객체 접근
